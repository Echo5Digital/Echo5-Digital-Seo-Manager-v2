...
export default function Tasks() {
  ...
  const { tasks, loading, fetchTasks, updateTask, deleteTask } = useTasksStore()
  ...
  const handleDeleteTask = async (task) => {
    if (!['Boss', 'Manager', 'Admin'].includes(user?.role)) {
  ...
  const canDeleteTasks = () => {
    return ['Boss', 'Manager', 'Admin'].includes(user?.role)
  }
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto" onClick={() => setShowModal(false)}>
          <div className="flex min-h-screen items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-xl shadow-2xl max-w-2xl w-full" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="bg-gray-50 px-6 py-4 rounded-b-xl flex justify-between gap-3">
                <div>
                  {canDeleteTasks() && (
                    <button
                      onClick={() => {
